...
The script uses `appendRow`, so it adds submissions below existing rows and does not change the header row. If the script changes later, create a new deployment version from **Manage deployments**.
...
3. In the Pages project, open **Settings → Environment variables** and add these encrypted variables for Production (and Preview if needed):
   ...
   - `FORM_ACCESS_CODE`: a private code shared only with authorized form users.
